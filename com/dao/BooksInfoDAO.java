...
public class BooksInfoDAO extends BaseDAO<BooksInfo> implements BooksInfoImpl {
    ...
    public void AddBooks(String s, String isbn, int typeid, String bookname, String author, String publish, Date publishdate, int publishtime,double unitprice) {
        String sql2 = "insert into `bookinformation` values (?,?,?,?,?,?,?,?,1)";
    ...
    public void ShowBooksISBNName() {
        String sql = "select ISBN,bookname,state from `bookinformation` ";
        List<BooksInfo> booksInfoList = doQueryResultList(sql);

        if (booksInfoList.size() != 0) {
            System.out.println("图书名称\t\t ISBN\t\t 是否可以借阅");
            for (BooksInfo bl : booksInfoList) {
                System.out.println(bl.getBookname() + " \t\t" + bl.getISBN() + " \t\t" + bl.isState());
            }
        } else {
            InputLimit.Warn("没有找到任何信息");
        }
    ...
    //借阅图书时改变图书状态
    @Override
    public void SetStateFalse(String s, String isbn) {
        String sql2 = "update  bookinformation set state=? where `ISBN`=?";
        doUpdate(sql2, false, isbn);
        new LogService().AOL("操作员[" + s + "]改变图书状态" + isbn + "");
    }

    //归还图书时改变图书状态
    @Override
    public void SetStateTrue(String s, String isbn) {
        String sql2 = "update  bookinformation set state=? where `ISBN`=?";
        doUpdate(sql2, true, isbn);
        new LogService().AOL("操作员[" + s + "]改变图书状态" + isbn + "");
    }

    @Override
    public boolean ConfirmBookState(String isbn) {
        String sql = "select state from bookinformation where ISBN=?";
        BooksInfo booksInfo = doQueryOneData(sql, isbn);

        //System.out.println("bookState：" + booksInfo.isState());

        return booksInfo.isState();
    }
}
